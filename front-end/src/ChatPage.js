...
function ChatPage() {
  ...
  */
  useEffect(() => {
    ...
    axios.get(`http://{$process.env.host}:3001/chatpage/${otherperson_username}`, {
      headers: {
  ...
  const getUser = async () => {
    try {
      const response = await axios.get('http://{$process.env.host}:3001/chatUser', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  function sendMessage(msg) {
    ...
    axios.post('http://{$process.env.host}:3001/chatpage2', msg_post)
      .then(response => {
